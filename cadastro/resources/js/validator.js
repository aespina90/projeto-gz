...
//Validação para ativar primeiro botão proximo
function validar1() {
...
function validar2() {
    ...
    if (document.getElementById("email").value.length > 0) {   
        ...
        alert('Campo e-mail não pode estár em branco.');
...
//Valida terceiro botão

function validar3() {
    var cep = form.cep.value;
    var cnpj = form.cpf_cnpj.value;
    var nomeFantasia = form.nomeFantasia.value;
    var razaoSocial = form.razaoSocial.value;
    cnpj = cnpj.replace.replace(/[^0-9]/g,'');

    if (nomeFantasia == "") {
        alert('Coloque o nome Fantasia da sua empresa!');
    ...
    if (razaoSocial == "") {
        alert('Coloque a Razão Social da sua empresa!');
        form.razaoSocial.focus();
        return false;
    }

    if ((cnpj.length <= 0)) {
        alert('CPF/CNPJ informado está em branco !');
        form.cpf_cnpj.focus();
        return false; 
    }
    ...
    if (cnpj.length >= 14){
        if (!validarCNPJ(cnpj)) {
            alert('CPF/CNPJ informado são inválidos !');
            form.cpf_cnpj.focus();
            form.cpf_cnpj.value = "";
            return false;
        }
...
//Retira mascara FAZER REGEX
function retiraMascara(cnpj, cep, fone) {
