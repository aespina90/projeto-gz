...
function EditarLinha(idLinha, cod, id){
	if(linhaEmEdicao == null){//Se linhaEmEdicao não for nulo...
		...
		celulas[1].innerHTML = '<input type="text" name="pergunta" value="'+celulas[1].innerHTML+'">';//Mostrar o campo texto permitindo a edição do nome do produto
		celulas[2].innerHTML = '<input type="text" name="r_certa" value="'+celulas[2].innerHTML+'">';//Mostrar o campo texto permitindo a edição do preço do produto
		celulas[3].innerHTML = '<input type="text" name="r_2" value="'+celulas[3].innerHTML+'">';//Mostrar o campo texto permitindo a edição do preço do produto
		celulas[4].innerHTML = '<input type="text" name="r_3" value="'+celulas[4].innerHTML+'">';//Mostrar o campo texto permitindo a edição do preço do produto
		celulas[5].innerHTML = '<input type="text" name="r_4" value="'+celulas[5].innerHTML+'">';//Mostrar o campo texto permitindo a edição do preço do produto
		celulas[6].innerHTML = '<input type="text" name="r_5" value="'+celulas[6].innerHTML+'">';//Mostrar o campo texto permitindo a edição do preço do produto
		celulas[7].innerHTML = '<a href="#" onclick="Atualizar(document.formulario.cod.value, document.formulario.pergunta.value, document.formulario.r_certa.value, document.formulario.r_2.value, document.formulario.r_3.value, document.formulario.r_4.value, document.formulario.r_5.value);"><img src="../../../images/ok1.jpg" alt="Atualizar" /></a>';//Monta os links que chamarão as funções para atualizar ou cancelar a edição da linha
		celulas[8].innerHTML = '<a href="#" onclick="Cancelar('+id+');"><img src="../../../images/no.jpg" alt="Cancelar" /></a>';//Insere um espaço na última célula
	}
...
function ExcluirLinha(idLinha, cod){
	if(!linhaEmEdicao){
		...
		if(confirm("Tem certeza que deseja Ativar/Desativar essa Pergunta?")){//Pergunta se a linha realmente deve ser excluída
...
function NovoRegistro(cod){
	...
	if(linhaEmEdicao){// Se linhas em edição for nulo...
		...
		novaLinha.className = 'linhas';//Define a classe de estilos que será usada na n ova linha
	}
	...
	novasCelulas[2].innerHTML = '<input type="text" name="r_certa">'; //insere o campo código
	novasCelulas[3].innerHTML = '<input type="text" name="r_2">'; //insere o campo código
	novasCelulas[4].innerHTML = '<input type="text" name="r_3">'; //insere o campo código
	novasCelulas[5].innerHTML = '<input type="text" name="r_4">'; //insere o campo código
	novasCelulas[6].innerHTML = '<input type="text" name="r_5">'; //insere o campo código
	novasCelulas[7].innerHTML = '<a href="#" onclick="Cadastrar(document.formulario.pergunta.value, document.formulario.r_certa.value, document.formulario.r_2.value, document.formulario.r_3.value, document.formulario.r_4.value, document.formulario.r_5.value, '+cod+');"><img src="../../../images/ok1.jpg" alt="Atualizar" /></a>'; //Monta os links que chamarão as funções para cadastrar e cancelar a inserção de uma nova linha
	novasCelulas[8].innerHTML = '<a href="#" onclick="CancelarInclusao();"><img src="../../../images/no.jpg" alt="Cancelar" /></a>';// Cria o link para cancelar a inserção de dados do formulário
}
...
function Cancelar(id){
	self.location.href="index.php?id="+id; //Direciona a página
}
...
function Atualizar(c, p ,rc ,r2 , r3, r4, r5){
	Aviso(1); //Exibe o aviso aguarde...
	var dados = ObtemDadosForm(p, rc, r2, r3, r4, r5);//Chama a função que montará a string com os dados que estarão na url
...
function Cadastrar(p, rc, r2, r3, r4, r5, cod){
	...
	var dados = ObtemDadosForm(p, rc, r2, r3, r4, r5); //Armazena a string com dados que comporão a url
...
function ObtemDadosForm(p, rc, r2, r3, r4, r5){
	parametros = "p="+p+"&r_c="+rc+"&r_2="+r2+"&r_3="+r3+"&r_4="+r4+"&r_5="+r5;//Define os parâmetros da url que será enviada
