...
// Images Directory
imagesDir = "backend/includes/lettera/icons/";

// CSS Directory
cssDir = "backend/includes/lettera/styles/";

// Popups Directory
popupsDir = "backend/includes/lettera/popups/";
...
// List of available block formats (not in use)
	...
	BlockFormats[0]  = "Endereço";
	BlockFormats[1]  = "Bulleted List";
	BlockFormats[2]  = "Definição";
	BlockFormats[3]  = "Termo de Definição";
	BlockFormats[4]  = "Lista de Diretórios";
	BlockFormats[5]  = "Formatado";
	BlockFormats[6]  = "Cabeçalho 1";
	BlockFormats[7]  = "Cabeçalho 2";
	BlockFormats[8]  = "Cabeçalho 3";
	BlockFormats[9]  = "Cabeçalho 4";
	BlockFormats[10] = "Cabeçalho 5";
	BlockFormats[11] = "Cabeçalho 6";
	BlockFormats[12] = "Menu em Lista";
	BlockFormats[13] = "Normal";
	BlockFormats[14] = "Lista Numerada";
...
//	buttonName2[1]  = "backcolor";
	...
	buttonName2[3] = "unorderedlist";
	buttonName2[4] = "orderedlist";
	buttonName2[5] = "outdent";
	buttonName2[6] = "indent";
	buttonName2[7]  = "seperator";
	buttonName2[8]  = "undo";
	buttonName2[9]  = "redo";
	buttonName2[10]  = "seperator";
	buttonName2[11]  = "inserttable";
	//buttonName2[12]  = "insertimage";
	buttonName2[12]  = "insertimage_url";
	buttonName2[13]  = "createlink";
...
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
	...
	"justifyleft":    ['Justifyleft',          'Alinhar a Esquerda',        imagesDir + 'justify_left.gif',       imagesDir + 'justify_left_on.gif'],
	"justifycenter":  ['Justifycenter',        'Alinhar ao Centro',      imagesDir + 'justify_center.gif',     imagesDir + 'justify_center_on.gif'],
	"justifyright":   ['Justifyright',         'Alinhar a Direita',       imagesDir + 'justify_right.gif',      imagesDir + 'justify_right_on.gif'],
	"justifyfull":    ['justifyfull',          'Justificar',       imagesDir + 'justify_full.gif',      imagesDir + 'justify_full_on.gif'],
	"unorderedlist":  ['InsertUnorderedList',  'Inserir Lista Desordenada',imagesDir + 'list_unordered.gif',     imagesDir + 'list_unordered_on.gif'],
	"orderedlist":    ['InsertOrderedList',    'Inserir Lista Ordenada',  imagesDir + 'list_ordered.gif',       imagesDir + 'list_ordered_on.gif'],
	"outdent":        ['Outdent',              'Recuar Parágrafo',            imagesDir + 'indent_left.gif',        imagesDir + 'indent_left_on.gif'],
	"indent":         ['Indent',               'Avançar Parágrafo',             imagesDir + 'indent_right.gif',       imagesDir + 'indent_right_on.gif'],
	...
	"forecolor":      ['ForeColor',            'Cor de Fonte',          imagesDir + 'forecolor.gif',          imagesDir + 'forecolor_on.gif'],
	"backcolor":      ['BackColor',            'Cor de FUndo',          imagesDir + 'backcolor.gif',          imagesDir + 'backcolor_on.gif'],
	...
	"insertimage_url":['InsertImage_URL',          'Inserir Caminho de Imagem',        imagesDir + 'insert_picture_url.gif',     imagesDir + 'insert_picture_on_url.gif'],
	"createlink":     ['CreateLink',           'Criar Link',         imagesDir + 'insert_hyperlink.gif',   imagesDir + 'insert_hyperlink_on.gif'],
	...
	"selectfont":     ['SelectFont',           'Escolher Fonte',         imagesDir + 'select_font.gif',        imagesDir + 'select_font_on.gif'],
	"selectsize":     ['SelectSize',           'Escolher Tamanho',         imagesDir + 'select_size.gif',        imagesDir + 'select_size_on.gif']
};
...
/* ---------------------------------------------------------------------- *\
  Function    : gera_lettera()
  Description : replace textarea with wysiwyg editor
  Usage       : gera_lettera("textarea_id");
  Arguments   : textarea_id - ID of textarea to replace
\* ---------------------------------------------------------------------- */
function gera_lettera(textareaID) {
      ...
      var buttonImage          = buttonObj[2];
		  ...
		  if (buttonName2[j] == "seperator") {
		    ...
		    toolbar2 += '<td style="width: 22px;">';
				toolbar2 += '<span id="HTMLMode' + n + '"><img src="'  +buttonImage+  '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="button" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\'' +buttonImageRollover+ '\';" onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\'' + buttonImage + '\';" unselectable="on"  width="20" height="20" style="display:none"></span>';
...
function formatText(id, n, selected) {
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	var formatIDs = new Array("FontSize","FontName","Bold","Italic","Underline","Subscript","Superscript","Strikethrough","Justifyleft","Justifyright","Justifycenter","justifyfull","InsertUnorderedList","InsertOrderedList","Indent","Outdent","ForeColor","BackColor","InsertImage","InsertImage_URL","InsertTable","CreateLink");
	...
	if (viewTextMode == 1 && disabled_id == 1) {
	  alert ("Você está em modo HTML. Esta ferramenta foi desabilitada. \nPara usá-la, retorne ao modo texto.");
	}
      ...
      var currentColor = _dec_to_rgb(document.getElementById("wysiwyg" + n).contentWindow.document.queryCommandValue(id));
		...
		// InsertImage
            ...
            window.open(popupsDir + 'insert_image.php?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=400,height=260');
	  }
	  
	   // InsertImage URL
	  else if (id == "InsertImage_URL") {
            window.open(popupsDir + 'insert_image_url.php?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=300,height=80');
	  }
	    ...
	    window.open(popupsDir + 'insert_hyperlink.php?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=310,height=110');
	  }
...
function outputFontSelect(n) {
  ...
  var FontSelectOn         = FontSelectObj[3];
	...
	FontSelectDropDown[n] += '<span id="Fonts' + n + '" class="dropdown" style="width: 155px;">';
...
function outputFontSizes(n) {
  ...
  var FontSizeOn         = FontSizeObj[3];
	...
	FontSizesDropDown[n] = '<table border="0" cellpadding="0" cellspacing="0"><tr><td onMouseOver="document.getElementById(\'selectSize' + n + '\').src=\'' + FontSizeOn + '\';" onMouseOut="document.getElementById(\'selectSize' + n + '\').src=\'' + FontSize + '\';"><img src="' + FontSize + '" id="selectSize' + n + '" width="66" height="20" onClick="showFontSizes(\'' + n + '\');" unselectable="on"><br>';
  FontSizesDropDown[n] += '<span id="Sizes' + n + '" class="dropdown" style="width: 265px;">';

	for (var i = 0; i <= FontSizes.length;) {
	  if (FontSizes[i]) {
      FontSizesDropDown[n] += '<button type="button" onClick="formatText(\'FontSize\',\'' + n + '\',\'' + FontSizes[i] + '\')\;hideFontSizes(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 245px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left" style="font-family: arial, verdana, helvetica;"><font size="' + FontSizes[i] + '">tamanho ' + FontSizes[i] + '</font></td></tr></table></button><br>';
    }
	  i++;
  }
