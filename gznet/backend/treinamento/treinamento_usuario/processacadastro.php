...
If($var_acao == "A"){
	$rs_sql	= 'SELECT * FROM '.$default['tabela'].' WHERE id="'.$var_id.'" AND tipo = "U"';
	...
	$rs_sql = "UPDATE ".$default['tabela']." SET nome = '".$var_nome."', datatreino = '".$var_datatreino."', conteudo = '".$var_conteudo."', status = '".$var_status."', datacadastro=datacadastro WHERE id = '".$var_id."' AND tipo = 'U'";
...
If($var_acao == "I"){
	$rs_sql = "INSERT INTO ".$default['tabela']." (nome, tipo, datatreino, conteudo, status, datacadastro)
			   VALUES('".$var_nome."', 'U', '".$var_datatreino."', '".$var_conteudo."', '".$var_status."', NOW())";
	$rs_query = mysql_query($rs_sql,$conexao['conexao'])or die(mysql_error());
...
If($var_acao == "E"){
	...
	$rs_linhas = mysql_num_rows($rs_query);
	if($rs_linhas > 0){ $rs_dados = mysql_fetch_array($rs_query); }
	$rs_sql = "DELETE FROM ".$default['tabela']." WHERE id = '".$var_id."' AND tipo = 'U'";
	$rs_query = mysql_query($rs_sql,$conexao['conexao'])or die(mysql_error());
